...
extract_sequences_fasta <- function(matrix, all_proteins, db, bacteria, id){
  if (db == 'COG'){
  ...
  names_equivalence$New_name = str_remove(names_equivalence$New_name, '_O.fna')
  ...
  if (bacteria %!in% 'All'){
    ...
    df = merge(df, names_equivalence, by.x = 'bacteria_name', by.y = 'New_name')
    df_sub = data.table(df)
    gene_list_one_bacteria = df_sub[df_sub$scientific_name %in% bacteria]$fasta_name
...
clustering_knn<- function(pca_data,  k){
  res.km <- kmeans(pca_data[,2:4], k, nstart = 25)

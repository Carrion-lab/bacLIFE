...
super_abs_pres_plot <- function(mtrx, groups, group1,group2, column){
  ...
  mtrx[2:ncol(mtrx)] <- mtrx[2:ncol(mtrx)] %>% mutate_all(~ if_else(. > 1, 1, .)) #Faster and less memory
  subset1 <- mtrx[,samples_1]
  subset2 <- mtrx[,samples_2]
  
  group1_means <- rowMeans(subset1[,1:ncol(subset1)])
  group2_means <- rowMeans(subset2[,1:ncol(subset2)])
...
extract_position <- function(str, patrn){
  split <- str_split(str, ',')
  split <- split[[1]]
  gene <- split[grep(patrn, split)]
...
subfunction <- function(lista){
  a = str_split(lista, '[|]')[[1]][2]
  return(a)
}
